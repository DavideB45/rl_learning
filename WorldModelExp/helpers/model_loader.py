...
def load_vq_vae(env:dict, codebook_size:int, code_depth:int, latent_dim:int, device) -> VQVAE:
	model = VQVAE(
	...
	model_path = env['models'] + f"vq_{latent_dim}_{code_depth}_{codebook_size}.pth"
	model.load_state_dict(torch.load(model_path, map_location=device))
	model.eval()
	return model
...
def save_vq_vae(env:dict, model:VQVAE):
	model_path = env['models'] + f"vq_{model.latent_dim}_{model.code_depth}_{model.codebook_size}.pth"
	torch.save(model.state_dict(), model_path)