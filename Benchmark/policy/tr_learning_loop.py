...
def main():
	...
	transformer = TransformerArc(CURRENT_ENV['a_size'], vq, EMB_SIZE, MAX_SEQ_LEN, NUM_HEADS, NUM_LAYERS, DROPOUT, best_device())
	agent = None
...
def tune_tr(model: TransformerArc, tr:DataLoader, vl:DataLoader, encoder: VQVAE, num_epocs:int=20, lr:float=5e-5, wd=5e-4) -> TransformerArc:
	model.vq = encoder
	optim = Adam(model.parameters(), lr=lr, weight_decay=wd)
	best_val_loss = float('inf')
	no_improvements = 0
	for epoch in range(num_epocs):
		...
		if err_vl['mse'] < best_val_loss:
			...
			no_improvements = 0
			save_transformer(CURRENT_ENV, model, cl=False, kl=False)
		else:
			no_improvements += 1
			if no_improvements >= 5:
				break
	del model
	return load_transformer(CURRENT_ENV, encoder, best_device(), EMB_SIZE, MAX_SEQ_LEN, NUM_HEADS, NUM_LAYERS, DROPOUT, cl=False, kl=False)
